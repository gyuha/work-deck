...
**renderer가 표시하는 원격·외부 콘텐츠는 신뢰할 수 없는 입력으로 취급한다.** 미리보기 모듈이 렌더링하는 파일 내용(특히 마크다운, [features/preview.md](features/preview.md) 3.2절)은 로컬이든 원격이든 실행 가능한 형태로 그리지 않는다 — 렌더링 결과에 Content-Security-Policy를 적용해 인라인 스크립트 실행과 임의 원격 리소스 로드를 기본 차단한다. 이 요구는 개별 기능 문서의 재량이 아니라 renderer 전체에 적용되는 아키텍처 수준 방침이다.
...
시크릿은 두 종류로 나뉘고 각각 다르게 다룬다. **저장된 시크릿**(safeStorage에 보관된 비밀번호·패스프레이즈)과 **사용자 입력 시크릿**(저장 없이 그 자리에서 한 번 쓰는 입력).

1. **복호화는 main에서만** — safeStorage API는 main 프로세스에서만 호출한다.
2. **저장된 시크릿 원문은 main 밖으로 나가지 않는다** — 복호화와 그것을 쓰는 인증은 main의 연결 모듈 내부에서만 수행한다. renderer는 "이 프로필에 저장된 시크릿이 있는지" 여부만 알 수 있으며, 저장된 시크릿 원문을 IPC로 받는 경로는 존재하지 않는다.
3. **사용자 입력 시크릿은 전용 경로로 1회 전달** — 저장된 시크릿이 없어 그 자리에서 입력받는 경우, 그 입력은 터미널 데이터 스트림과 분리된 마스킹 입력 오버레이에서 받아 전용 IPC 채널로 main에 **한 번** 전달된 뒤 양쪽 메모리에서 즉시 폐기된다(로그·스크롤백에 기록하지 않는다). 이 1회성 전달은 2항의 불변식이 다루는 대상(이미 저장된 시크릿의 반복 노출)이 아니다 — 입력 상세는 [features/connections.md](features/connections.md) 5.2절.

연결을 열 때의 시크릿 흐름(저장된 시크릿이 있는 경우)은 다음과 같다. 저장된 시크릿 원문이 IPC 경계를 넘지 않는다는 점이 핵심이다.
