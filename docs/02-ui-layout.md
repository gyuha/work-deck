...
사이드바·북마크에서 항목을 열 때는 항상 중복 검사를 한다. **같은 대상이 이미 열려 있으면 새 탭을 만들지 않고 기존 콘텐츠 탭에 포커스한다.** 새 탭이 필요한 경우에는 현재 포커스된 분할 영역에 열린다(분할하지 않았다면 단일 영역). 로컬 터미널 탭은 사이드바 항목이 아니라 명령으로 생성되므로 이 중복 검사의 대상이 아니다([features/terminal.md](features/terminal.md) 2.1절).

"같은 대상"의 식별 기준은 탭 타입별로 다음과 같다. 경로 비교는 모두 **정규화된 절대 경로**로 한다 — 끝 슬래시를 제거하고, macOS는 APFS 기본값(대소문자 비구분)에 맞춰 대소문자를 구분하지 않고 비교하며(Windows/Linux는 대소문자 구분), 심볼릭 링크는 따라가지 않고 주어진 경로 문자열 그대로 정규화한다. 원격 경로는 연결이 응답한 경로 문자열을 기준으로 하며 서버 측 realpath 해석은 하지 않는다.

| 콘텐츠 탭 타입 | 같은 대상 판정 기준 |
|----------------|---------------------|
| 파일 목록 탭 (로컬) | 정규화된 폴더 경로가 같음 |
| 파일 목록 탭 (원격) | 연결 프로필과 정규화된 경로가 모두 같음 |
| 미리보기 탭 | 정규화된 파일 경로가 같음 (원격 파일은 연결 프로필 포함) |
| SSH 터미널 탭 | 연결 프로필이 같음 (종료됨·끊김 상태의 기존 탭도 대상에 포함 — [features/terminal.md](features/terminal.md) 5.2·5.3절) |

같은 연결이라도 SSH 터미널 탭과 원격 파일 목록 탭은 서로 다른 대상이다 — 하나의 연결에서 두 탭이 각각 열려 공존할 수 있다. 중복 검사는 분할 여부와 무관하게 워크스페이스 전체를 대상으로 하며, 일치하는 기존 탭이 반대쪽 분할 영역에 있으면 포커스가 그 영역으로 이동한다.

**파일 목록 탭의 하드 중복 금지 완화**: 파일 목록 탭 안에서의 폴더 탐색([features/file-manager.md](features/file-manager.md) 1.1절)은 새 탭을 만들지 않고 정체성을 바꾸므로, 탐색만으로도 같은 대상의 탭이 여러 개 존재하는 상태가 자연히 생길 수 있다(예: 분할 양쪽에서 각자 같은 폴더로 탐색해 들어온 경우). 이는 오류가 아니다 — 파일 목록 탭에 한해 위 중복 검사는 **사이드바·북마크로 새로 열 때만** 적용되는 소프트 규칙으로, 이미 화면에 여러 일치 탭이 있으면 그중 **가장 최근에 활성화된 탭**에 포커스한다. 여기에 더해 파일 목록 탭은 컨텍스트 메뉴의 "새 탭에서 열기"로 중복 검사를 건너뛰고 명시적으로 같은 경로의 탭을 하나 더 열 수 있다 — 분할 양쪽을 같은 폴더에서 시작해 각자 하위로 탐색해 가는 듀얼 패널 워크플로우를 위한 탈출구다. 미리보기 탭과 SSH 터미널 탭은 하드 중복 금지를 그대로 유지한다(탐색으로 정체성이 바뀌지 않으므로 완화할 이유가 없다).
...
2. **터미널 키 충돌 최소화, 플랫폼별로 다른 기준** — 터미널 탭이 포커스일 때는 아래 표에 명시된 소수의 앱 전역 단축키를 제외한 모든 키 입력을 터미널로 전달한다. macOS는 앱 전역 단축키를 `Cmd` 조합으로만 구성해 셸·TUI 프로그램이 쓰는 `Ctrl` 조합과 자연히 겹치지 않는다. **Windows/Linux는 다르다** — 단일 `Ctrl` 조합은 셸·readline 관습(예: `Ctrl+W`=직전 단어 삭제, `Ctrl+C`=인터럽트)과 정면으로 겹치므로, Windows/Linux의 앱 전역 단축키는 `Ctrl+Shift` 조합을 기본으로 한다.
3. **관습 준수** — 탭·분할 조작은 VSCode 관습을, 파일 작업 키(F5/F6)는 듀얼 패널 파일 관리자 관습을 따른다. 사이드바 뷰 전환의 `Cmd`(`Ctrl`)`+숫자` 조합은 VSCode에서는 에디터 그룹 포커스 이동에 쓰이는 조합과 같다 — 뷰 전환에 재배정한 것은 관습과 어긋남을 인지한 선택이며(WorkDeck에는 에디터 그룹 대신 사이드바 뷰가 3개뿐이라는 판단), 유지한다.
4. **터미널 탭 닫기는 확인을 거친다** — 실행 중인 자식 프로세스가 있거나 SSH 세션이 활성인 터미널 탭을 닫으려 하면 확인 대화상자를 표시한다([features/terminal.md](features/terminal.md) 5.1절). 단축키로 닫을 때도 예외 없이 적용된다.
...
| 다음 / 이전 콘텐츠 탭 | `Cmd+Shift+]` / `Cmd+Shift+[` | `Ctrl+Shift+Tab` / `Ctrl+Shift+Shift+Tab`\* |
| 활성 탭 닫기 (확인 필요 시 대화상자, 방침 4) | `Cmd+W` | `Ctrl+Shift+W` |
| 분할 토글 (분할 실행 / 해제) | `Cmd+\` | `Ctrl+Shift+\` |
| 분할 영역 간 포커스 이동 | `Cmd+Opt+←/→` | `Ctrl+Shift+Alt+←/→` |
| 활성 탭을 반대 분할 영역으로 이동 | `Cmd+Opt+Shift+←/→` | `Ctrl+Shift+Alt+Shift+←/→`\* |

\* 조합이 부자연스러운 항목은 구현 단계에서 실제 OS·터미널 관습을 재조사해 확정한다 — 여기서 확정하는 것은 "Windows/Linux 앱 전역 단축키는 `Ctrl` 단독이 아니라 `Ctrl+Shift` 기반"이라는 방침(2번)이며, 개별 키 조합이 아니다.
