...
**MVP에서는 연결당 SSH 터미널 탭(세션)을 1개만 허용한다.** 이는 [02-ui-layout.md](../02-ui-layout.md) 3장의 중복 검사 규칙("SSH 터미널 탭의 같은 대상 판정 기준 = 연결 프로필이 같음")의 직접 귀결이다 — 같은 연결에서 "터미널로 열기"를 다시 실행하면 새 탭을 만들지 않고 기존 SSH 터미널 탭에 포커스한다. 연결당 복수 세션을 허용하려면 이 판정 기준 자체를 바꿔야 하므로 MVP 범위 밖이다.
...
세션의 전체 수명은 다섯 상태로 정리된다. 탭이 열리면 세션이 **생성**되고, PTY 스폰 또는 SSH 인증·셸 채널 수립이 진행되는 동안 **연결중**이다. 성공하면 **활성** — 입출력이 흐르는 정상 상태다. 활성에서 벗어나는 길은 세 가지다: 셸 프로세스가 스스로 끝나면 **종료됨**(탭 잔존, 5.2절), SSH에서 원격 끊김이 감지되면 **끊김**(재연결 버튼으로 다시 연결중으로 진입, 5.3절), 사용자가 탭을 닫으면 어느 상태에서든 세션이 소멸한다(5.1절). SSH의 최초 연결 실패도 끊김 상태로 표시되어 같은 재연결 버튼으로 재시도한다. 끊김과 재연결 루프는 SSH 터미널 전용이며, 로컬 터미널은 생성 → 연결중 → 활성 → 종료됨의 경로만 가진다.
...
stateDiagram-v2
    ...
    connecting --> disconnected : 연결 실패 (SSH)
    active --> exited : 셸 프로세스 종료 (exit)
    active --> disconnected : 원격 끊김 감지 (SSH 전용)
    disconnected --> connecting : 재연결 버튼 (수동)
    active --> [*] : 탭 닫기 = 세션 종료
    disconnected --> [*] : 탭 닫기
    exited --> [*] : 탭 닫기
