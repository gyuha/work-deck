...
미리보기 탭은 읽기 전용이다. 편집·저장 기능이 없고, 미리보기에서 파일 내용이 변경되는 경우는 없다. 내용은 탭을 열 때 읽은 시점 기준으로 표시하며, 이후 파일이 바뀌어도 자동으로 갱신하지 않는다(MVP 범위 제한).
...
- **렌더 표시 (기본)** — 마크다운 파일은 렌더된 결과를 기본으로 표시한다.
- **원본 토글** — 탭 안의 토글로 렌더 표시와 원본(마크다운 소스) 표시를 전환한다. 원본 표시는 3.1절의 텍스트/코드 동작(마크다운 구문 강조, 인코딩 처리, 대용량 처리)을 그대로 따른다. 토글 상태는 탭 단위이며, 새로 여는 마크다운 미리보기 탭은 항상 렌더 표시로 시작한다.
